...
class StructParser {
    ...
    public function __construct($fp, $line, $uniqueName, $moduleName, $structName, $preStructs, $preEnums, $namespaceName, $preNamespaceEnums, $preNamespaceStructs) {
        $this->fp = $fp;
    ...
    public function parse() {
        ...
        // 先把积累下来的三个部分处理掉
        $structClassStr = $this->getStructClassHeader('\\classes') . 'class ' . $this->structName . " extends \TARS_Struct {" . $this->returnSymbol;
        ...
        $construct = $this->tabSymbol . 'public function __construct() {' . $this->returnSymbol . $this->doubleTab . "parent::__construct('" . $this->uniqueName . '_' . $this->structName . "', self::\$_fields);" . $this->returnSymbol . $this->extraContructs . $this->extraExtInit . $this->tabSymbol . '}' . $this->returnSymbol;
    ...
    public function structBodyParseLine() {
        ...
        while (1) {
            ...
            if ($this->state == 'init') {
                // 有可能是换行
                if ($char == '{' || Utils::isSpace($char) || $char == '\r' || $char == '\x0B' || $char == '\0') {
                    continue;
                } elseif ($char == "\n") {
                    break;
    ...
    public function writeStructLine($tag, $requireType, $type, $valueName, $wholeType, $defaultValue) {
        ...
        if (Utils::isBasicType($type)) {
            $this->fields .= $this->doubleTab . 'self::' . strtoupper($valueName) . ' => array(' . $this->returnSymbol . $this->tripleTab . "'name'=>'" . $valueName . "'," . $this->returnSymbol . $this->tripleTab . "'required'=>" . $requireFlag . ',' . $this->returnSymbol . $this->tripleTab . "'type'=>" . Utils::getRealType($type) . ',' . $this->returnSymbol . $this->tripleTab . '),' . $this->returnSymbol;
