...
class InterfaceParser {
    ...
    public function __construct($fp, $line, $namespaceName, $moduleName, $interfaceName, $preStructs, $preEnums, $servantName, $preNamespaceEnums, $preNamespaceStructs) {
        $this->fp = $fp;
    ...
    public function getFileHeader($prefix = '') {
        return "<?php\n\nnamespace " . $this->namespaceName . $prefix . ';' . $this->doubleReturn . 'use Tars\\client\\CommunicatorConfig;' . $this->returnSymbol . 'use Tars\\client\\Communicator;' . $this->returnSymbol . 'use Tars\\client\\RequestPacket;' . $this->returnSymbol . 'use Tars\\client\\TUPAPIWrapper;' . $this->returnSymbol . $this->returnSymbol;
    }

    public function getInterfaceBasic() {
        return $this->tabSymbol . 'protected $_communicator;' . $this->returnSymbol . $this->tabSymbol . 'protected $_iVersion;' . $this->returnSymbol . $this->tabSymbol . 'protected $_iTimeout;' . $this->returnSymbol . $this->tabSymbol . "public \$_servantName = \"$this->servantName\";" . $this->doubleReturn . $this->tabSymbol . 'public function __construct(CommunicatorConfig $config) {' . $this->returnSymbol .

            $this->doubleTab . 'try {' . $this->returnSymbol . $this->tripleTab . '$config->setServantName($this->_servantName);' . $this->returnSymbol . $this->tripleTab . '$this->_communicator = new Communicator($config);' . $this->returnSymbol . $this->tripleTab . '$this->_iVersion = $config->getIVersion();' . $this->returnSymbol . $this->tripleTab . '$this->_iTimeout = empty($config->getAsyncInvokeTimeout())?2:$config->getAsyncInvokeTimeout();' . $this->returnSymbol . $this->doubleTab . '} catch (\\Exception $e) {' . $this->returnSymbol . $this->tripleTab . 'throw $e;' . $this->returnSymbol . $this->doubleTab . '}' . $this->returnSymbol . $this->tabSymbol . '}' . $this->doubleReturn;
    }
    ...
    public function generateFuncBody($inParams, $outParams, $returnInfo) {
        ...
        $bodySuffix = $this->doubleTab . 'catch (\\Exception $e) {' . $this->returnSymbol . $this->tripleTab . 'throw $e;' . $this->returnSymbol . $this->doubleTab . '}' . $this->returnSymbol;

        $bodyMiddle = $this->tripleTab . '$requestPacket = new RequestPacket();' . $this->returnSymbol . $this->tripleTab . '$requestPacket->_iVersion = $this->_iVersion;' . $this->returnSymbol . $this->tripleTab . '$requestPacket->_funcName = __FUNCTION__;' . $this->returnSymbol . $this->tripleTab . '$requestPacket->_servantName = $this->_servantName;' . $this->returnSymbol . $this->tripleTab . '$encodeBufs = [];' . $this->doubleReturn;
        ...
        foreach ($inParams as $param) {
            ...
            if (Utils::isVector($type)) {
                $vecFill = $this->tripleTab . '$' . $valueName . '_vec = ' . $this->getExtType($param['wholeType'],
                        $valueName) . ';' . $this->returnSymbol . $this->tripleTab . 'foreach($' . $valueName . ' as ' . '$single' . $valueName . ') {' . $this->returnSymbol . $this->quardupleTab . '$' . $valueName . '_vec->pushBack($single' . $valueName . ');' . $this->returnSymbol . $this->tripleTab . '}' . $this->returnSymbol;
                $bodyMiddle .= $vecFill;
            ...
            elseif (Utils::isMap($type)) {
                $mapFill = $this->tripleTab . '$' . $valueName . '_map = ' . $this->getExtType($param['wholeType'],
                        $valueName) . ';' . $this->returnSymbol . $this->tripleTab . 'foreach($' . $valueName . ' as ' . '$key => $value) {' . $this->returnSymbol . $this->quardupleTab . '$' . $valueName . '_map->pushBack([$key => $value]);' . $this->returnSymbol . $this->tripleTab . '}' . $this->returnSymbol;
                $bodyMiddle .= $mapFill;
        ...
        $bodyMiddle .= $this->tripleTab . '$requestPacket->_encodeBufs = $encodeBufs;' . $this->doubleReturn;
        ...
        foreach ($outParams as $param) {
            ...
            if (Utils::isBasicType($type)) {
                ...
                if (Utils::isVector($type) || Utils::isMap($type)) {
                    $bodyMiddle .= $this->tripleTab . "\$$name = TUPAPIWrapper::" . $unpackMethods . '("' . $name . "\",{$index}," . $this->getExtType($param['wholeType'],
                            $name) . ',$sBuffer,$this->_iVersion);' . $this->returnSymbol;
                } // 如果是struct
                elseif (Utils::isStruct($type, $this->preStructs)) {
        ...
        if ($returnInfo['type'] !== 'void') {
            if (Utils::isVector($returnInfo['type']) || Utils::isMap($returnInfo['type'])) {
                $bodyMiddle .= $this->tripleTab . 'return TUPAPIWrapper::' . $returnUnpack . '("",0,' . $this->getExtType($returnInfo['wholeType'],
                        $valueName) . ',$sBuffer,$this->_iVersion);' . $this->doubleReturn . $this->doubleTab . '}' . $this->returnSymbol;
                ...
                $bodyMiddle .= $this->tripleTab . 'return $returnVal;' . $this->doubleReturn . $this->doubleTab . '}' . $this->returnSymbol;
                ...
                $bodyMiddle .= $this->tripleTab . 'return TUPAPIWrapper::' . $returnUnpack . '("",0,$sBuffer,$this->_iVersion);' . $this->doubleReturn . $this->doubleTab . '}' . $this->returnSymbol;
            }
        } else {
